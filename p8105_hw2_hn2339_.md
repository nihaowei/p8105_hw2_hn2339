...
``` r
library(readxl)
Precipation_2016 = read_excel("data /HealthyHarborWaterWheelTotals2017-9-26.xlsx", sheet = 4, range = "A1:B15") %>%
  janitor::clean_names() %>%
  filter(!is.na(x_1))
```
...
``` r
library(readxl)
Precipation_2017 = read_excel("data /HealthyHarborWaterWheelTotals2017-9-26.xlsx", sheet = 3, range = "A1:B15") %>%
  janitor::clean_names() %>%
  filter(!is.na(x_1))
```
